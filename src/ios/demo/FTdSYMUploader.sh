...
#
######################################################
# 1. Script integration into Xcode project's Target
######################################################
#
...
# Whether to only generate dSYM zip file, 1=only package dSYM zip and do not upload, 0=upload. You can search FT_DSYM_ZIP_FILE in the script output log to find the DSYM_SYMBOL.zip file path
FT_DSYM_ZIP_ONLY=1
#
#
# Whether to upload in Debug mode compilation, 1=upload 0=do not upload, default is not to upload
# UPLOAD_DEBUG_SYMBOLS=0
#
# # Whether to upload in simulator compilation, 1=upload 0=do not upload, default is not to upload
# UPLOAD_SIMULATOR_SYMBOLS=0
#
# # Only upload during Archive operation, 1=support Archive upload 0=upload for all Release mode compilations
...
#
#######################################################
# 2. Script processes based on input parameters
#######################################################
#
# #Enter basic app information in the command line
# # Upload symbol table file
# sh  FTdSYMUpload.sh <datakit_address> <app_id> <version> <env> <dataway_token> <dSYMBOL_src_dir>
# or
# # Only compress the symbol table file
...
#  - `<version>`: App's `version`, value of `CFBundleShortVersionString`
...
#  - `<dSYM_ZIP_ONLY>`: Whether to only package dSYM files into a zip file. Optional. 1=do not upload, only package dSYM Zip, 0=upload. You can search `FT_DSYM_ZIP_FILE` in the script output log to find the Zip file path
#
# --- CONTENT OF SCRIPT ---
#
...
# Check if the app's dSYM file is empty. If it is, loop and wait for 10s, then exit if still empty
...
function runInXcode(){
    ...
    # Assemble the default recognized version information (format is CFBundleShortVersionString, e.g.: 1.0)
    if [ ! "${FT_VERSION}" ]; then
    FT_VERSION="${BUNDLE_SHORT_VERSION}"
    fi
    
    ## Check if simulator build allows symbol upload
    ...
    ## Check if it is a Release mode build
    ...
    ## Check if it is an Archive operation
...
# Determine whether it is in Xcode environment based on Xcode environment variables
...
while [[ $# -gt 0 ]]; do
    ...
    case ${key} in
        ...
        ;;
        *)
        # Ignore unknown parameters or errors
        shift
        ;;
    esac
done
